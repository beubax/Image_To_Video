...
@click.command()
@click.option("-r", "--dataset-root", type=click.Path(exists=True), required=False, help="path to dataset.")
@click.option("-a", "--annotation-path", type=click.Path(exists=True), required=False, help="path to dataset.")
...
# )
    ...
    train_set = foz.load_zoo_dataset("hmdb51", split="train")
    test_set = foz.load_zoo_dataset("hmdb51", split="test")
