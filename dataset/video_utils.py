...
class VideoClips:
    ...
    def __init__(
        ...
        self.compute_clips(clip_length_in_frames, frames_between_clips, frame_sample_rate, frame_rate)
    ...
    def compute_clips_for_video(
        video_pts: torch.Tensor, num_frames: int, step: int, frame_sample_rate: int,  fps: Optional[float], frame_rate: Optional[float] = None
        ...
        clips = unfold(video_pts, num_frames * dilation, step * dilation)
        while not clips.numel() and dilation >= 1:
            dilation //= 2
            clips = unfold(video_pts, num_frames * dilation, step * dilation)

        dilation = 1 if dilation == 0 else dilation
        if not clips.numel():
            warnings.warn(
        ...
        if isinstance(_idxs, slice):
            ...
            idxs = unfold(_idxs, num_frames * dilation, step * dilation)
        return clips, idxs

    def compute_clips(self, num_frames: int, step: int, frame_sample_rate: int, frame_rate: Optional[float] = None) -> None:
        """
        ...
        Args:
            ...
            frame_sample_rate (int): sample one frame every `frame_sample_rate` frames
        ...
        for video_pts, fps in zip(self.video_pts, self.video_fps):
            clips, idxs = self.compute_clips_for_video(video_pts, num_frames, step, frame_sample_rate, fps, frame_rate)
    ...
    def get_clip(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any], int]:
        ...
        return video[::video.shape[0]//self.num_frames], audio, info, video_idx
    ...
    def __setstate__(self, d: Dict[str, Any]) -> None:
        ...
        self.compute_clips(self.num_frames, self.step, self.frame_sample_rate, self.frame_rate)
